...
* [Helper Scripts](#helper-scripts)
  * [Notes About the Output Files and Locally Generated Files](#notes-about-the-output-files-and-locally-generated-files)
    * [CSVs](#csvs)
    * [Log and Script Output Files](#log-and-script-output-files)
    * [Local Cache Files and Downloaded Files](#local-cache-files-and-downloaded-files)
...
#### CSVs
The CSVs are generated in the root directory of the repository when you run the `main.py` script, excluding `task6_values.csv` which was manually generated using the Task 5 CSV.
...
* `task7_values.csv` - Generated for [Task 7](#task-7-comp6700-only)
...
In addition to the log files, there are a few cache files and python files that are downloaded for various tasks when running `main.py`. The first file is the `df_data.pkl`, which is a pickle cache that contains the Pandas data frames that have been downloaded from the AIDev hugging face dataset. This file was generated to help save loading times when running the script multiple times. This pickle cache will take up about 1.3GB of space.

The second set of files that are downloaded locally will be located in the `./local-github-filestore/` directory, which contains all the Python files that could be downloaded for [Task 7](#task-7-comp6700-only) bandit scanning. This will take up a few megabytes of space for the download of all of these files.
...
***NOTE:*** _The `pr_commit_details.csv` and `task7_values.csv` were noted to have changed at least once during the duration of this project. This is most likely due to the fact that the **pr_commit_details** dataset gets added to more frequently for PRs as commits are made by the AI agents. This change directly impacts Task 7 since it utilizes the values in `pr_commit_details.csv`, so the `task7_values.csv` will always change when the commit details CSV changes._
